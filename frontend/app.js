// frontend/app.js
const API_BASE = (window.location.hostname.includes("localhost") ? "http://localhost:5000/api" : "https://ezeevote.onrender.com/api");
...
function hideAll(){ ["register-section","otp-section","login-section","forgot-section","reset-section","profile-section","voting-section","results-section"].forEach(id=>document.getElementById(id).style.display="none"); }
...
document.getElementById("register-form").addEventListener("submit", async e=>{
  ...
  if(data.userId){ document.getElementById("otp-email").value = email; hideAll(); document.getElementById("otp-section").style.display="block"; }
});
...
document.getElementById("login-form").addEventListener("submit", async e=>{
  ...
  const data = await res.json();
  alert(data.message || data.error);
  if(data.token){ setToken(data.token); currentUserId = data.userId; loadProfile(); loadCandidates(); }
});
...
async function loadCandidates(){
  ...
  (data.candidates || []).forEach(c=>{
    ...
    div.innerHTML = `<img src="images/${c.image}" /><h3>${c.name}</h3><p>${c.party}</p><button class="vote-btn">Vote</button>`;
...
async function viewResults(){
  ...
  new Chart(document.getElementById("resultsChart"), { type:"bar", data:{ labels:names, datasets:[{ label:"Votes", data:votes }] }, options:{ responsive:true, maintainAspectRatio:false }});
